...
class FCOS(Model):
    def __init__(self, config, data_provider, **kwargs):
    ...
    def call(self, image):
        ...
        if self.is_bottleneck:
            stack_residual_unit_fn = _residual_bottleneck
        else:
            stack_residual_unit_fn = _residual_block
        residual_block = pool1
        for i in range(self.block_list[0]):
            residual_block = stack_residual_unit_fn(residual_block, self.filters_list[0], 1, 'block1_unit' + str(i + 1))
        endpoints.append(residual_block)
        ...
        c3 = _bn_activation_conv(endpoints[-3], 256, 1, 1)
        c4 = _bn_activation_conv(endpoints[-2], 256, 1, 1)
        c5 = _bn_activation_conv(endpoints[-1], 256, 1, 1)
        p5 = _get_pyramid(c5, 256)
        p4, top_down = _get_pyramid(c4, 256, p5)
        p3, _ = _get_pyramid(c3, 256, top_down)
        p6 = _bn_activation_conv(p5, 256, 3, 2)
        p7 = _bn_activation_conv(p6, 256, 3, 2)

        p3conf, p3reg, p3center = _detect_head(p3, self.num_classes)
        p4conf, p4reg, p4center = _detect_head(p4, self.num_classes)
        p5conf, p5reg, p5center = _detect_head(p5, self.num_classes)
        p6conf, p6reg, p6center = _detect_head(p6, self.num_classes)
        p7conf, p7reg, p7center = _detect_head(p7, self.num_classes)
        ...
        return (p3conf, p3reg, p3center) 
# %%
data = tf.ones(shape=[8,512,512,3], dtype=tf.float32)
fcos = FCOS(config, trainset_provider)
fcos(data)
fcos.summary()
fcos